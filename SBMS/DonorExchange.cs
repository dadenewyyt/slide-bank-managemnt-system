...
namespace MSBMS
{
    public partial class DonorExchange : Form
    {
       private static NLog.Logger logger = NLog.LogManager.GetCurrentClassLogger();
                ...
                using (SqlConnection connection = new SqlConnection(DBConnectionSingltonServices.connectionString))
                {
                    ...
                    using (SqlCommand command = new SqlCommand())
                    {
                        ...
                        finally
                        {
                            ...
                            clear();
                            logger.Info("Donor has+"+txt_barcode_scan_in.Text+" is saved");
        ...
        private void btn_find_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    if (String.Compare(row.Cells[3].Value.ToString(),searchValue.ToString().Trim())==0)
                    {
                        row.Selected = true;
                        found = true;
